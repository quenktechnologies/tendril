...
class MiddlewareStage {
    ...
    execute() {
        ...
        return future_1.fromCallback(cb => {
            let init = either_1.right(undefined);
            let result = record_1.reduce(modules, init, (prev, mData) => {
                if (prev.isLeft())
                    return prev;
                let exApp = mData.app;
                ...
                if (emwares.isLeft())
                    return either_1.left(emwares.takeLeft());
...
const getMiddlewareByNames = (mData, names) => {
    ...
    if (allFound)
        return either_1.right(results.map(r => r.get()));
    let missing = results.map((r, idx) => r.isNothing() ? names[idx] : '');
    return either_1.left(namesNotFoundErr(mData.path, missing.filter(name => name)));
};
// TODO: Migrate to App. See issue #45
const getMiddlewareByName = (mData, name) => {
    let result = maybe_1.fromNullable(mData.middleware.available[name]);
    if (result.isJust())
        return result;

        ...
        return (0, future_1.fromCallback)(cb => {
            (0, record_1.map)(modules, m => {
                if (m.template &&
                    ...
                    let conf = (0, record_1.merge)(defaultOptions, m.template.app.csrf.token);
                    ...
                    if (conf.on && conf.on.failure)
                        m.app.use(m.module.runInCSRFErrorContext([conf.on.failure]));
                }
                if ((0, data_1.getValue)(m, isEnabled) === true)
...
const setCSRFToken = (r) => (0, api_1.doAction)(function* () {
    let token = yield (0, csrf_1.getToken)();
    yield prs.set(exports.PRS_CSRF_TOKEN, token);
    yield prs.set(exports.PRS_VIEW_CSRF_TOKEN, token);
    return (0, control_1.next)(r);
});
const isEnabled = (m) => m.template &&
    m.template.app &&
    m.template.app.csrf &&
    m.template.app.csrf.token &&
    m.template.app.csrf.token.enable;

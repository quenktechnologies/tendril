...
class SessionStage {
    ...
    execute() {
        ...
        return (0, future_1.sequential)((0, record_1.mapTo)(modules, m => (0, future_1.doFuture)(function* () {
            if (m.template &&
                m.template.app &&
                m.template.app.session &&
                m.template.app.session.enable) {
                let conf = (0, record_1.merge)(defaultOptions, m.template.app.session);
                ...
                m.app.use(session((0, record_1.merge)(conf.options, { store })));
 ...
 * @private
 */
const handleSessionTTL = (req, _, next) => {
    if (req.session &&
        (0, type_1.isObject)(req.session[session_1.SESSION_DATA]) &&
        (0, type_1.isObject)(req.session[session_1.SESSION_DESCRIPTORS])) {
        let session = req.session;
        let descs = session[session_1.SESSION_DESCRIPTORS];
        descs = (0, record_1.map)(descs, (d, k) => {
            if ((d.ttl != null) && (0, type_1.isNumber)(d.ttl)) {
                if (d.ttl === 0) {
                    (0, session_1.deleteSessionKey)(session, k);
                }
                else {
                    d.ttl = d.ttl - 1;
                }
...
exports.handleSessionTTL = handleSessionTTL;

 ...
 * from a Template.
 */
exports.getAvailableMiddleware = (t) => (t.app && t.app.middleware && t.app.middleware.available) ?
    record_1.map(t.app.middleware.available, m => m.provider.apply(null, m.options || [])) : {};
/**
 * getEnabledMiddleware extracts the list of enabled middleware.
 */
exports.getEnabledMiddleware = (t) => (t.app && t.app.middleware && t.app.middleware.enabled) ?
    t.app.middleware.enabled : [];
/**
 * getRoutes provides the route function from a Template.
 */
exports.getRoutes = (t) => (t.app && t.app.routes) ? t.app.routes : () => [];
/**
 * getShowFun provides the "show" function of a Template.
 *
 * If not specified, the parent show function is used.
 */
exports.getShowFun = (t, parent) => (t.app && t.app.views) ?
 ...
 */
exports.getServerConf = (t, defaults) => record_1.merge(defaults, (t.server == null) ? {} : t.server);
/**
 * getHooks provides the hook handlers configuration from a template.
 */
exports.getHooks = (t) => (t.app && t.app.on) ? t.app.on : {};
/**
 * getConnections provides the connections from a template.
 */
exports.getConnections = (t) => {
    if (t.connections == null)
        return {};
    return record_1.reduce(t.connections, {}, (p, c, k) => {
        if (c.connector == null)
            return p;
        p[k] = (c.options != null) ?

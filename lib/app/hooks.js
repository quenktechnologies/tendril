...
class Dispatcher {
    ...
    init() {
        let { app } = this;
        return future_1.parallel(record_1.values(record_1.map(app.modules, (m) => {
            let mHooks = maybe_1.fromNullable(m.hooks.init);
            if (mHooks.isNothing())
                return future_1.pure(undefined);
            let hooks = mHooks.get();
            return Array.isArray(hooks) ?
                future_1.sequential(hooks.map(f => f(app)))
                    .map(function_1.noop) :
                hooks(app);
        })))
            .map(function_1.noop);
    ...
    connected() {
        let { app } = this;
        return future_1.parallel(record_1.values(record_1.map(app.modules, (m) => {
            let mHooks = maybe_1.fromNullable(m.hooks.connected);
            if (mHooks.isNothing())
                return future_1.pure(undefined);
            let hooks = mHooks.get();
            return (Array.isArray(hooks)) ?
    ...
    started() {
        let { app } = this;
        return future_1.parallel(record_1.values(record_1.map(app.modules, (m) => {
            let mHooks = maybe_1.fromNullable(m.hooks.started);
            if (mHooks.isNothing())
                return future_1.pure(undefined);
            let hooks = mHooks.get();
            return Array.isArray(hooks) ?

...
class Get extends __1.Api {
    ...
    exec(ctx) {
        return future_1.pure(this.next(ctx.request.session.get(this.key)));
    }
}
exports.Get = Get;
/**
...
class GetOrElse extends __1.Api {
    ...
    exec(ctx) {
        return future_1.pure(this.next(ctx.request.session.getOrElse(this.key, this.value)));
    }
}
...
class Exists extends __1.Api {
    ...
    exec(ctx) {
        return future_1.pure(this.next(ctx.request.session.exists(this.key)));
    }
}
...
class EnabledSessionStorage {
    ...
    static fromExpress(r) {
        return type_1.isObject(r.session) ?
    ...
    isEnabled() {
        return type_1.isObject(this.data);
    }
    get(key) {
    ...
    remove(key) {
        ...
        this.data[exports.SESSION_DATA] =
            record_1.rcompact(path.set(key, undefined, target));
        delete descs[key];
        this.data[exports.SESSION_DESCRIPTORS] = descs;
        return this;
    }
    ...
    save() {
        return type_1.isFunction(this.data.save) ?
            future_1.fromCallback(cb => this.data.save(cb)) :
            future_1.pure(undefined);
    }
    regenerate() {
        return type_1.isFunction(this.data.regenerate) ?
            future_1.fromCallback(cb => this.data.regenerate(cb)) :
            future_1.pure(undefined);
    }
    destroy() {
        return type_1.isFunction(this.data.destroy) ?
            future_1.fromCallback(cb => this.data.destroy(cb)) :
            future_1.pure(undefined);
    }
}
 ...
 */
const set = (key, value, desc = {}) => free_1.liftF(new Set(key, value, desc, undefined));
exports.set = set;
/**
 ...
 */
const regenerate = () => free_1.liftF(new Regenerate(undefined));
exports.regenerate = regenerate;
/**
 * destroy the session.
 */
const destroy = () => free_1.liftF(new Destroy(undefined));
exports.destroy = destroy;
/**
 * Save session data.
 *
 ...
 * of the request.
 */
const save = () => free_1.liftF(new Save(undefined));
exports.save = save;

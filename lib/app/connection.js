...
class Pool {
     ...
     **/
    get(key) {
        return (0, maybe_1.fromNullable)(this.conns[key]);
    }
    /**
     * open all the connections in the pool.
     */
    open() {
        return (0, future_1.parallel)((0, record_1.mapTo)(this.conns, c => c.open())).map(function_1.noop);
    }
    /**
     * close all the connections in the pool.
     */
    close() {
        return (0, future_1.parallel)((0, record_1.mapTo)(this.conns, c => c.close())).map(() => {
            this.conns = {};
        });
    }
}
 ...
 */
const getUserConnection = (name) => (0, future_1.doFuture)(function* () {
    let mConn = pool.get(name);
    if (mConn.isNothing())
        return (0, future_1.pure)(mConn);
    let conn = yield mConn.get().checkout();
    return (0, future_1.pure)((0, maybe_1.just)(conn));
});
 ...
 */
const unsafeGetUserConnection = (name) => (0, exports.getUserConnection)(name).map(m => m.get());
